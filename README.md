...
`src/app/icon.svg` is "RC" in **IBM Plex Mono Regular (400)** — the same face and weight as
the header wordmark and the Request Compute button — at the same `0.12em` tracking, paper
on ink.
...
To regenerate: extract the `R` and `C` outlines from IBM Plex Mono Regular with
